...
import { DrinkResult, PaginatedQuery, QueryDrinksArgs, QueryDrinksHistoryArgs, QueryEntriesArgs } from '../__generated__/graphql'
...
describe('queryResolvers', () => {
  ...
  describe('node', () => {
    ...
    describe('drink history node', () => {
      beforeEach(async () => {
        ...
        node = await queryResolvers.node?.({}, { id: mockId }, ctx, info) as DrinkHistoryModel
      })
    ...
    describe('entry node', () => {
      beforeEach(async () => {
        ...
        node = await queryResolvers.node?.({}, { id: mockId }, ctx, info) as ResolvedEntry
      })
  ...
  describe('drinks', () => {
    ...
    beforeEach(async () => {
      ...
      res = await queryResolvers.drinks?.({}, mockArgs, ctx, info) as PaginatedQuery
    })
  ...
  describe('entry', () => {
    ...
    beforeEach(async () => {
      mockId = toCursorHash('Entry:123')
      res = await queryResolvers.entry?.({}, { id: mockId }, ctx, info) as ResolvedEntry
    })
  ...
  describe('entries', () => {
    ...
    beforeEach(async () => {
      mockArgs = { first: 2, after: '123' }
      res = await queryResolvers.entries?.({}, mockArgs, ctx, info) as PaginatedQuery
    })
  ...
  describe('drinkHistory', () => {
    ...
    beforeEach(async () => {
      mockId = toCursorHash('DrinkHistory:123')
      res = await queryResolvers.drinkHistory?.({}, { id: mockId }, ctx, info) as DrinkHistoryModel
    })
  ...
  describe('drinksHistory', () => {
    ...
    beforeEach(async () => {
      ...
      res = await queryResolvers.drinksHistory?.({}, { ...mockArgs }, { prisma }, info) as PaginatedQuery
    })
  ...
  describe('user requests', () => {
    ...
    describe('me', () => {
      beforeEach(async () => {
        res = await queryResolvers.me?.({}, {}, ctx, info) as User
      })
    ...
    describe('user', () => {
      test('makes a call to `prisma.user` with the given argument', async () => {
        ...
        await queryResolvers.user?.({}, { id: mockId }, ctx, info)
    ...
    describe('users', () => {
      ...
      test('makes a call to `prisma.user` to fetch all users', async () => {
        await queryResolvers.users?.({}, {}, ctx, info)
        expect(prisma.user.findMany).toHaveBeenCalled()
      })

      test('returns users with hashed ids', async () => {
        const res = await queryResolvers.users?.({}, {}, ctx, info)
